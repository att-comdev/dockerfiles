...
def main():
   parser = argparse.ArgumentParser(description='Queries Prometheus and Keeps Nagios config Updated.')
   parser.add_argument('--prometheus_api', metavar='prometheus_api', type=str, required=True, help='Prometheus query API with scheme, host and port')
   parser.add_argument('--update_seconds', metavar='update_seconds', type=int, required=False, default=60, help='When run as daemon, sleep time')
   parser.add_argument('--hosts', metavar='get_nagios_hosts', type=str, required=False, help='Output Nagios Host definition to stdout')
   parser.add_argument('--hostgroups', metavar='get_nagios_hostgroups', type=str, required=False, help='Output Nagios Hostgroup definition to stdout')
   parser.add_argument('--object_file_loc', metavar='object_file_loc', type=str,
                       required=False, default="/opt/nagios/etc/conf.d/prometheus_discovery_objects.cfg",
                       help='Output Nagios Host definition to stdout')
   parser.add_argument('-d', action='store_true', help="Flag to run as a deamon")
...
def reload_nagios():
   command = ["/usr/sbin/service", "nagios", "restart"];
   subprocess.call(command, shell=False)

def get_nagios_hostgroups(prometheus_api):
   nagios_hostgroups = []
   for host, labels in get_nagios_hostgroups_dictionary(prometheus_api).iteritems():
      for label in labels:
          nagios_hostgroup_defn = NAGIOS_HOSTGROUP_FORMAT.format(hostgroup=label)
   ...
   try:
      ...
      for uname in unames_json['data']['result']:
         ...
         if hostgroup_dictionary[host_name]:
            hostgroups = hostgroups + "," + ",".join(hostgroup_dictionary[host_name])
         nagios_host_defn = NAGIOS_HOST_FORMAT.format(host_name=host_name, host_ip=host_ip, hostgroups=hostgroups)
...
def query_prometheus(prometheus_api, query):
   parsed_url = urlparse.urlparse(prometheus_api, 'http')
   url = "{}/api/v1/query".format(parsed_url.geturl())
   params = {"query": query}
